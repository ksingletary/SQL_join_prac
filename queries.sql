...
SELECT first_name, last_name, COUNT(*) FROM owners JOIN vehicles ON owner_id = owners.id GROUP BY first_name, last_name ORDER BY COUNT(*) ASC;
...
SELECT first_name, last_name, ROUND(AVG(price)) as average_price, COUNT(*) FROM owners JOIN vehicles ON owners.id = vehicles.owner_id GROUP BY first_name, last_name HAVING COUNT(owner_id) > 1 AND ROUND(AVG(price)) > 10000 ORDER BY AVG(price), COUNT(*) DESC;
